...
namespace PetrolStation.Forms.Forms
{
	public partial class CustomerForm : General.SuperForm
	{
		...
		Common.BLL.Entity.PetrolStation.Owner 			ownerModel;			
		 

		Common.BLL.Logic.PetrolStation.Car				lCar;
		Common.BLL.Entity.PetrolStation.Car			carModel;		
		
		Common.BLL.Entity.PetrolStation.CarOwner		carOwnerModel;

		Common.BLL.Logic.PetrolStation.Plate			lPlate;
		Common.BLL.Entity.PetrolStation.Plate			plateModel;
		
		Common.BLL.Logic.PetrolStation.LegalOwner		lLegalOwner;
		Common.BLL.Entity.PetrolStation.LegalOwner		legalOwnerModel;
	
		Common.BLL.Entity.PetrolStation.Tag			tagModel;
		Common.BLL.Logic.PetrolStation.Tag				lTag;
			
		Common.BLL.Logic.PetrolStation.CarTag			lCarTag;
		Common.BLL.Entity.PetrolStation.CarTag			carTagModel;
		...
		#region Methods		
		public CustomerForm (Common.BLL.Entity.PetrolStation.CarOwner  carOwnerModel = null)
		{
			InitializeComponent ();
		...
		{
			bool result = false;
			Common.BLL.Logic.PetrolStation.Tag		lTagCheck		= new	Common.BLL.Logic.PetrolStation.Tag(Common.Enum.EDatabase.PetrolStation);
            Common.BLL.Entity.PetrolStation.Tag    tagModelCheck = new Common.BLL.Entity.PetrolStation.Tag()
			{
				tag = tag
			};
			...
			Common.BLL.Logic.PetrolStation.CarTag		lCarTagCheck		= new	Common.BLL.Logic.PetrolStation.CarTag(Common.Enum.EDatabase.PetrolStation);
			Common.BLL.Entity.PetrolStation.CarTag		carTagModelCheck	= new Common.BLL.Entity.PetrolStation.CarTag()
			{
		...
		private void prepare ()
		{
			...
			{
				carOwnerModel	= new Common.BLL.Entity.PetrolStation.CarOwner ();
				ownerModel		= new  Common.BLL.Entity.PetrolStation.Owner();			
				legalOwnerModel	= new Common.BLL.Entity.PetrolStation.LegalOwner();
				plateModel		= new Common.BLL.Entity.PetrolStation.Plate();
				tagModel		= new Common.BLL.Entity.PetrolStation.Tag();
				carTagModel		= new Common.BLL.Entity.PetrolStation.CarTag();
				carModel		= new Common.BLL.Entity.PetrolStation.Car();
			...
			else
			{
				...
				//Load Owner
				lOwner	= new Common.BLL.Logic.PetrolStation.Owner(Common.Enum.EDatabase.PetrolStation);
				ownerModel	= new Common.BLL.Entity.PetrolStation.Owner()
				{
				...
				BaseBLL.General.FormModelHelper<Common.BLL.Entity.PetrolStation.Owner>.fillControl(ownerDataGroupBox, ownerModel);
				birthdateMaskedTextBox.Text = Convert.ToDateTime(ownerModel.birthdate).ToString("yyyy/MM/dd");

				// Load Car
				lCar	= new Common.BLL.Logic.PetrolStation.Car(Common.Enum.EDatabase.PetrolStation);
			 	carModel	= new Common.BLL.Entity.PetrolStation.Car()
				{
				...
				BaseBLL.General.FormModelHelper<Common.BLL.Entity.PetrolStation.Car>.fillControl(carDataGroupBox, carModel);
				...
				//Load Plate
				lPlate	= new Common.BLL.Logic.PetrolStation.Plate (Common.Enum.EDatabase.PetrolStation);
				plateModel	= new Common.BLL.Entity.PetrolStation.Plate()
				{
					id = carModel.plateId
				};
				CommandResult	resultPlate = lPlate.read(plateModel, "id");
				BaseBLL.General.FormModelHelper<Common.BLL.Entity.PetrolStation.Plate>.fillControl(plateDataGroupBox, plateModel);
				...
				if (carOwnerModel.type)
				{
					...
					lLegalOwner	= new Common.BLL.Logic.PetrolStation.LegalOwner (Common.Enum.EDatabase.PetrolStation);
					legalOwnerModel	= new Common.BLL.Entity.PetrolStation.LegalOwner()
					{
						carOwnerId = carOwnerModel.id
					};
					CommandResult	resultLegalOwner = lLegalOwner.read(legalOwnerModel,"carOwnerId");
					BaseBLL.General.FormModelHelper<Common.BLL.Entity.PetrolStation.LegalOwner>.fillControl(legalOwnerDataGroupBox, legalOwnerModel);
				}
				else 
					realRadioButton.Checked = true;		
		
				//Load CarTag
				lCarTag = new Common.BLL.Logic.PetrolStation.CarTag (Common.Enum.EDatabase.PetrolStation);
				carTagModel = new Common.BLL.Entity.PetrolStation.CarTag ()
				{
				...
				if (resultCarTag.status == BaseDAL.Base.EnumCommandStatus.success)
				{
					...
					{
                        //Load Tag
                        lTag = new Common.BLL.Logic.PetrolStation.Tag(Common.Enum.EDatabase.PetrolStation);
                        tagModel = new Common.BLL.Entity.PetrolStation.Tag()
                        {
                             id = carTagModel.tagId
                        };
                        CommandResult resultTag = lTag.read(tagModel, "id");
                        BaseBLL.General.FormModelHelper<Common.BLL.Entity.PetrolStation.Tag>.fillControl(tagDataGroupBox, tagModel);                       
		...
		{
			// Sexuality
			Common.BLL.Logic.PetrolStation.HC__Sexuality	lSex	= new Common.BLL.Logic.PetrolStation.HC__Sexuality (Common.Enum.EDatabase.PetrolStation);
			...
			//Car Type
			Common.BLL.Logic.PetrolStation.Base__CarType	lCarType	= new Common.BLL.Logic.PetrolStation.Base__CarType (Common.Enum.EDatabase.PetrolStation);
			DataTable 	resultType			= lCarType.allData("", "", false).model as DataTable;		
			carTypeComboBox.fillByTable (resultType, "id", "type");

			//Car Fuel
			Common.BLL.Logic.PetrolStation.Base__CarFuel	lCarFuel	= new Common.BLL.Logic.PetrolStation.Base__CarFuel (Common.Enum.EDatabase.PetrolStation);
			DataTable resultFuel			= lCarFuel.allData ("", "", false).model as DataTable;
			carFuelComboBox.fillByTable (resultFuel, "id", "fuel");			

			//Car Level
			Common.BLL.Logic.PetrolStation.Base__CarLevel	lCarLevel	= new Common.BLL.Logic.PetrolStation.Base__CarLevel (Common.Enum.EDatabase.PetrolStation);
			DataTable 	resultLevel			= lCarLevel.allData ("", "", false).model as DataTable;		
			carLevelComboBox.fillByTable (resultLevel, "id", "levelcar");

			//Car System
			Common.BLL.Logic.PetrolStation.Base__CarSystem		lCarSystem	= new Common.BLL.Logic.PetrolStation.Base__CarSystem (Common.Enum.EDatabase.PetrolStation);
			DataTable	resultSystem		= lCarSystem.allData ("", "", false).model as DataTable;			
			carSystemComboBox.fillByTable (resultSystem, "id", "system");

			//Plate type 
			Common.BLL.Logic.PetrolStation.Base__PlateType		lPlateType		= new Common.BLL.Logic.PetrolStation.Base__PlateType (Common.Enum.EDatabase.PetrolStation);
		...
		{
			BaseBLL.General.FormModelHelper<Common.BLL.Entity.PetrolStation.Tag>.fillModel (tagDataGroupBox,tagModel);
			...
			if (opResult.status == BaseDAL.Base.EnumCommandStatus.success)
			{
				switch (step)
				{
					case 0 :
						{ 		
							...
							if (!checkOwner())
							{
								...
								BaseBLL.General.FormModelHelper<Common.BLL.Entity.PetrolStation.Owner>.fillModel (ownerDataGroupBox, ownerModel);
					...
					case 1 :
						{ 							
							// Fill car Model							
							BaseBLL.General.FormModelHelper<Common.BLL.Entity.PetrolStation.Car>.fillModel (carDataGroupBox, carModel);
					...
					case 3 :
						{
							...
							if (legalRadioButton.Checked)
							{
								...
								if (!checkLegalOwner())
									BaseBLL.General.FormModelHelper<Common.BLL.Entity.PetrolStation.LegalOwner>.fillModel (legalOwnerDataGroupBox, legalOwnerModel);
							}
							else 
		...
		{
			bool result = false;
			Common.BLL.Entity.PetrolStation.Owner	modelOwner	= new Common.BLL.Entity.PetrolStation.Owner ()
			{
				nationalCode = ownerModel.nationalCode					
			};

			//Common.BLL.Entity.PetrolStation.Car	modelCar	= new Common.BLL.Entity.PetrolStation.Car()
			//{
